...
class AdapterRecyclerInsuline(private val clickListener: (InsulineData) -> Unit) :
    RecyclerView.Adapter<AdapterRecyclerInsuline.MyViewHolder>() {
    private val dataList = ArrayList<InsulineData>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        val binding: DiabeteCardviewInsulineBinding = DataBindingUtil.inflate(
            layoutInflater,
            R.layout.diabete_cardview_insuline,
            parent,
            false
        )
